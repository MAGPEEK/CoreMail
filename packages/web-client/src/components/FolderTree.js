...
function FolderItem({ folder, selected, isSystem, hasChildren, expanded, onToggleExpand, onSelect, onContextMenu, label, indent, isDragDisabled, contextKey = '', }) {
    ...
    return (_jsxs("div", { ref: setDropRef, className: `relative rounded-sm transition-all duration-150 ${isOver ? 'ring-2 ring-accent ring-inset bg-accent/15 scale-[1.02] animate-drop-pulse' : ''}`, children: [selected && (_jsx("span", { className: "absolute left-0 top-1 bottom-1 w-[3px] bg-accent rounded-r animate-slide-in-left", "aria-hidden": true })), _jsxs("div", { ref: setDragRef, ...(dragDisabled ? {} : attributes), ...(dragDisabled ? {} : listeners), onClick: onSelect, onContextMenu: onContextMenu, style: { paddingLeft: 6 + indent * 14 }, className: `group w-full flex items-center gap-1.5 pr-3 py-1.5 text-sm rounded-sm transition-all duration-150 ${dragDisabled ? 'cursor-pointer' : 'cursor-grab active:cursor-grabbing'} ${selected ? 'bg-accent/10 text-accent font-medium' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 hover:translate-x-0.5'} ${isDragging ? 'opacity-40' : ''}`, role: "button", tabIndex: 0, children: [hasChildren ? (_jsx("button", { onClick: (e) => { e.stopPropagation(); onToggleExpand(); }, onPointerDown: (e) => e.stopPropagation(), className: "w-4 h-4 flex items-center justify-center text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 shrink-0", children: expanded ? _jsx(ChevronDown, { size: 12 }) : _jsx(ChevronRight, { size: 12 }) })) : (_jsx("span", { className: "w-4 h-4 shrink-0" })), _jsx(Icon, { size: 15, className: "shrink-0 transition-transform duration-150 group-hover:scale-110", style: folder.color ? { color: folder.color } : undefined }), folder.color && (_jsx("span", { className: "shrink-0 w-1.5 h-1.5 rounded-full", style: { backgroundColor: folder.color }, "aria-hidden": true })), _jsx("span", { className: "flex-1 text-left truncate", children: label }), folder.unreadCount > 0 && (_jsx(AnimatedCounter, { value: folder.unreadCount, className: "text-xs font-bold text-accent" }))] })] }));
}
export function FolderTree({ onNewMail }) {
    ...
    const renderFolderTree = (folder, indent, contextKey = 'tree') => {
        ...
        return (_jsxs("div", { children: [_jsx(FolderItem, { folder: folder, 
                    ...
                    selected: selectedFolderSource !== 'fav' && folder.id === selectedFolderId, isSystem: isSystem, hasChildren: hasChildren, expanded: expanded, onToggleExpand: () => toggleExpand(folder.id), onSelect: () => setSelectedFolder(folder.id, 'tree'), onContextMenu: handleContextMenu(folder), label: folderLabel(folder), indent: indent, contextKey: contextKey }), hasChildren && expanded && (_jsx("div", { children: children.map((c) => renderFolderTree(c, indent + 1, contextKey)) }))] }, `${contextKey}|${folder.id}`));
    };
    ...
    void lang;
    return (_jsxs("aside", { className: "w-52 shrink-0 bg-gray-50 dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 flex flex-col h-full", children: [_jsx("div", { className: "p-3", children: _jsxs("button", { onClick: onNewMail, className: "btn-primary w-full justify-center group/newmail animate-halo", children: [_jsx(Plus, { size: 15, className: "transition-transform duration-200 group-hover/newmail:rotate-90" }), t('new_mail')] }) }), _jsxs("nav", { className: "flex-1 overflow-y-auto px-1 pb-3", children: [favorites.length > 0 && (_jsxs(_Fragment, { children: [_jsx(SectionHeader, { label: t('favorites'), collapsed: favoritesCollapsed, onToggle: toggleFavorites }), !favoritesCollapsed && (_jsx("div", { className: "space-y-0.5", children: favorites.map((f) => (_jsx(FolderItem, { folder: f, 
                                    // selected nur, wenn die Auswahl IM Favoriten-Bereich getätigt wurde
                                    selected: selectedFolderSource === 'fav' && f.id === selectedFolderId, isSystem: SYSTEM_SET.has(f.name), hasChildren: false, expanded: false, onToggleExpand: () => { }, onSelect: () => setSelectedFolder(f.id, 'fav'), onContextMenu: handleContextMenu(f), label: folderLabel(f), indent: 0, contextKey: "fav", isDragDisabled: true }, `fav-${f.id}`))) }))] })), _jsx(SectionHeader, { label: t('folders'), collapsed: folderTreeCollapsed, onToggle: toggleFolderTree, action: _jsx("button", { onClick: () => setDialog({ kind: 'createRoot' }), className: "text-gray-400 hover:text-gray-700 dark:hover:text-gray-200", title: "Neuer Ordner", children: _jsx(Plus, { size: 12 }) }) }), !folderTreeCollapsed && (_jsx("div", { className: "space-y-0.5", children: systemFolders.map((f) => renderFolderTree(f, 0)) }))] }), menu && _jsx(ContextMenu, { x: menu.x, y: menu.y, items: menu.items, onClose: () => setMenu(null) }), dialog?.kind === 'createRoot' && (_jsx(PromptDialog, { title: "Neuer Ordner", label: "Name", placeholder: "z. B. Wichtige Mails", confirmText: "Erstellen", onCancel: () => setDialog(null), validate: (v) => /[/\\]/.test(v) ? 'Keine / oder \\ erlaubt' : null, onConfirm: async (name) => {

...
function MessageRow({ msg, selected, checked, onClick, onToggleCheck, onContextMenu, onQuickAction, density, folders, }) {
    ...
    return (_jsxs("div", { ref: setNodeRef, ...attributes, ...listeners, onClick: onClick, onContextMenu: onContextMenu, className: `w-full text-left px-3 ${py} border-b border-gray-100 dark:border-gray-700 transition-colors relative group cursor-pointer ${selected
            ? 'bg-accent/10'
            : checked
                ? 'bg-accent/5'
                : 'hover:bg-gray-50 dark:hover:bg-gray-800'} ${isDragging ? 'opacity-30' : ''}`, role: "button", tabIndex: 0, children: [_jsxs("div", { className: "flex items-start gap-2.5", children: [_jsx("button", { onClick: onToggleCheck, className: `mt-0.5 w-4 h-4 rounded-sm border flex items-center justify-center shrink-0 transition-all ${checked
                            ? 'bg-accent border-accent text-white'
                            : 'border-gray-300 dark:border-gray-600 opacity-0 group-hover:opacity-100 hover:border-accent'}`, "aria-label": "Ausw\u00E4hlen", children: checked && _jsx("span", { className: "text-[10px] leading-none", children: "\u2713" }) }), _jsx("button", { onClick: (e) => { e.stopPropagation(); onQuickAction('flag'); }, className: "mt-0.5 shrink-0", "aria-label": "Kennzeichnen", children: isFlagged
                            ? _jsx(Flag, { size: 14, className: "fill-red-500 text-red-500" })
                            : _jsx(Flag, { size: 14, className: "text-gray-300 dark:text-gray-600 hover:text-red-400" }) }), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsxs("div", { className: "flex items-center justify-between gap-2", children: [_jsxs("span", { className: `text-sm truncate ${isUnread ? 'font-semibold text-gray-900 dark:text-gray-100' : 'text-gray-700 dark:text-gray-300'}`, children: [isPinned && _jsx(Pin, { size: 11, className: "inline -mt-0.5 mr-1 text-accent" }), msg.fromAddr] }), _jsxs("div", { className: "flex items-center gap-1 shrink-0", children: [hasAttachments && _jsx(Paperclip, { size: 12, className: "text-gray-400" }), isSnoozed && _jsx(Clock, { size: 12, className: "text-amber-500" }), _jsx("span", { className: "text-xs text-gray-400", children: formatDate(msg.date) })] })] }), _jsx("p", { className: `text-sm truncate mt-0.5 ${isUnread ? 'font-medium text-gray-800 dark:text-gray-200' : 'text-gray-600 dark:text-gray-400'}`, children: msg.subject || '(kein Betreff)' })] }), _jsxs("div", { className: "absolute right-2 top-1/2 -translate-y-1/2 hidden group-hover:flex items-center gap-0.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded shadow-sm", children: [_jsx("button", { onClick: (e) => { e.stopPropagation(); onQuickAction('archive'); }, className: "p-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300", title: "Archivieren", children: _jsx(Archive, { size: 13 }) }), _jsx("button", { onClick: (e) => { e.stopPropagation(); onQuickAction('delete'); }, className: "p-1.5 hover:bg-red-50 dark:hover:bg-red-900/30 text-gray-600 dark:text-gray-300 hover:text-red-600", title: "L\u00F6schen", children: _jsx(Trash2, { size: 13 }) }), _jsx("button", { onClick: (e) => { e.stopPropagation(); onQuickAction('read'); }, className: "p-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300", title: isUnread ? 'Als gelesen markieren' : 'Als ungelesen markieren', children: isUnread ? _jsx(MailOpen, { size: 13 }) : _jsx(Mail, { size: 13 }) })] })] }), _jsx("span", { className: "hidden", children: folders.length })] }));
}
export function MessageList({ folderId }) {
    ...
    if (isLoading) {
        return (_jsx("div", { className: "w-72 shrink-0 flex items-center justify-center text-gray-400 text-sm", children: "Lade Nachrichten\u2026" }));
    }

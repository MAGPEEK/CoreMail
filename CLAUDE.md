...
- **Live-Server**: `84.247.191.198` (Contabo VPS, Ubuntu 24.04, 4 Cores, 7.8 GB RAM) — läuft v3.18.7; Deploy: `cd /opt/coremail && docker compose pull coremail && docker compose up -d coremail`
...
## Aktuelle Version 3.18.7 — Highlights

**v3.18.7** — Doppel-Feature: (1) **Spam-UX (Outlook-Style)** — `Message.spamScore` persistiert, Spam-Banner in MessageReader (rot ≥6.0 / amber ≥3.0 / im Junk-Ordner) mit Score-Anzeige und „Als Junk"/„Kein Spam"-Inline-Button, ShieldAlert-Badge in MessageList. (2) **Audit-Log v2** — SHA-256-Signatur für CSV/PDF/JSON-Exports (`X-CoreMail-Signature`-Header), neuer JSON-Export für SIEM (Splunk/Sentinel/ELK), Anomalie-Endpoint `/anomalies` mit Burst-Detection (≥50 Aktionen/5min), Fehler-Burst (≥10), kritische Aktionen (delete/settings/oauth/etc.) und Off-Hours-Activity. Meta-Logging gegen Insider-Threats: GET-Routen für Audit-Log-Exports werden auch auditiert, Immutability-Garantie dokumentiert. Frontend mit JSON-Button, Anomalien-Banner (60s auto-refresh), Signatur-Hinweis.
...
*Letzte Aktualisierung: 2026-05-24 (v3.18.2 — UI-Cleanup Outlook-Wording + Button-Fix; v3.18.1 — Compose-Window Gmail-Style Redesign; v3.18.0 — Outlook-Style Inbox Rules; v3.17.50 — Quelltext-Modal)*

...
|-----------|-------------|--------|
| `coremail` | `magpeek/coremail-app:3.17.29` | Alle Node.js-Services + MWA/BCP-Frontends (kein nginx!) |
...
- **Live-Server**: `84.247.191.198` (Contabo VPS, Ubuntu 24.04, 4 Cores, 7.8 GB RAM) — läuft v3.17.29; Deploy: `cd /opt/coremail && docker compose pull coremail && docker compose up -d coremail`
...
## Aktuelle Version 3.17.29 — Highlights
...
`{ name: displayName.trim(), address: email }`. Neues `ensureRfc5322Headers()` in
relay.ts als Sicherheitsnetz (prüft alle 3 Pflicht-Header vor MX-Zustellung).
...
*Letzte Aktualisierung: 2026-05-22 (v3.17.29 — Fix: RFC 5322 — From & Message-ID Header fehlend → Gmail 550 5.7.1 behoben)*
